...
class Analysis(GenomicBinSettings):
    ...
    def get_ks(self, vector_id, matrix_id):
        ...
        quartiles = [[], [], [], []]
        total_entries = len(sort_order)
        values = list(flcm.count_matrix.df['All bins'])
        for i, index in enumerate(sort_order):
            quartiles[math.floor(4*i/total_entries)].append(
                    values[index])

        statistic, critical_values, significance = anderson_ksamp(quartiles)
        return {
            'statistic': statistic,
    ...
    def get_unsorted_ks(self, matrix_id):
        ...
        total_entries = len(flcm.count_matrix.df['All bins'])
