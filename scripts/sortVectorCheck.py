...
class SortVectorCheck(object):
    ...
    def check_columns(self):
        ...
        with open(self.sort_vector_fn) as f:
            for line in f:
                if len(line.split("\t")) != 2:
                    column_check = False
                if self.is_float(line.strip().split('\t')[1]) == False:
                    float_check = False
    ...
    def check_ids(self):
        with open(self.sort_vector_fn) as sort_vector_lines, open(self.feature_bed_fn) as feature_bed_lines:
            for sort_vector_line, feature_bed_line in zip(sort_vector_lines, feature_bed_lines):
